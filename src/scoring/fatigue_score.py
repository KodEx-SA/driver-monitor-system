...
Normal/Warning/Critical) and, alert_manager.py (what to actually
do about it).
"""
...
from src.scoring.perclos import PerclosTracker
...
class FatigueScorer:
    ...
    def update(
        ...
        self._update_perclos(avg_ear, baseline_ear, ts)
        ...
        return FatigueScore(perclos=perclos, yawn_count_in_window=yawn_count, score=score)

    def _update_perclos(self, avg_ear: float, baseline_ear: float, ts: float) -> None:
